...
const currentImage = props.verifyOtpBannerImage
    ? `/storage/logo/${props.verifyOtpBannerImage}`
    : null;
...
<template>
  <div class="position-relative d-inline-block">
    <label for="image" class="d-block">
      <img
        :src="preview ? preview : verifyOtpBannerImage ? `/storage/logo/${verifyOtpBannerImage}` : 'placeholder.png'"
        class="img-thumbnail border border-dark rounded w-100"
        style="width: 100px; height: 100px; object-fit: cover;"
        alt="verify Banner Image"
      />
    </label>
